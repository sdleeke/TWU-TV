...
class MediaPlayer {
    ...
    func observe()
    {
        ...
        playerTimerReturn = player?.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(1,Constants.CMTime_Resolution), queue: DispatchQueue.main, using: { [weak globals] (time:CMTime) in
            globals?.playerTimer()
        })
